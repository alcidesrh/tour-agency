...
<script>
    ...
    export default {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'New Icon' : 'Edit Icon: '
            },
            ...mapGetters({
                deletedItem: 'icon/del/deleted',
                errorList: 'icon/list/error',
                errorCreate: 'icon/create/error',
                errorUpdate: 'icon/update/updateError',
                errorDelete: 'icon/del/error',
                items: 'icon/list/items',
                loading: 'icon/list/loading',
                view: 'icon/list/view',
                created: 'icon/create/created',
                deleteLoading: 'icon/del/loading',
                updateLoading: 'icon/update/updateLoading',
                createLoading: 'icon/create/loading'
            })
        },
        watch: {
            dialog(val) {
        ...
        methods: {
            ...
            deleteItem(item) {
                ...
                if (confirm('Are you sure you want to delete?')) {
                    ...
                    this.$store.dispatch('icon/del/delete', item).then(
                        () => {
                            if(this.flag){
                                this.flag = false;
                                return;
            ...
            save() {
                ...
                if (this.editedIndex > -1) {
                    ...
                    this.$store.dispatch('icon/update/update', {
                    ...
                    this.$store.dispatch('icon/create/create', itemAux).then(
                        () => {
                            if(this.flag){
                                this.flag = false;
                                return;
